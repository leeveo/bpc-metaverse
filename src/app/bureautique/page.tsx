...
export default function Bureautique() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const response = await axios.get('/api/check-auth-bureautique');
        if (response.status === 200) {
          setIsAuthenticated(true);
        } else {
          router.push('/login-bureautique');
        }
      } catch (error) {
        router.push('/login-bureautique');
      }
  ...
  const handleLogout = async () => {
    try {
      await axios.post('/api/logout-bureautique');
      router.push('/work');
    } catch (error) {
  ...
  const roomData = imagesData.find(data => data.link === "/bureautique"); // Find the room data
