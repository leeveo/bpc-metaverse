...
export default function Dynabuy() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const response = await axios.get('/api/check-auth-dynabuy');
        if (response.status === 200) {
          setIsAuthenticated(true);
        } else {
          router.push('/login-dynabuy');
        }
      } catch (error) {
        router.push('/login-dynabuy');
      }
  ...
  const handleLogout = async () => {
    try {
      await axios.post('/api/logout-dynabuy');
      router.push('/work');
    } catch (error) {
  ...
  const roomData = imagesData.find(data => data.link === "/dynabuy"); // Find the room data
