...
export default function LoginBureautiquePage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('/api/authenticate-bureautique', { email, password });
      if (response.status === 200) {
        router.push('/bureautique');
      }
    } catch (error) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
      <form onSubmit={handleSubmit} style={{ width: '300px', padding: '20px', border: '1px solid #ccc', borderRadius: '10px' }}>
        <h1>Connectez-vous à la salle Bureautique</h1>
